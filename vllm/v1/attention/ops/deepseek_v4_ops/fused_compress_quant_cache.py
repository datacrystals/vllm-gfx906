...
# =============================================================================
# ROCm fp16 path (head=512 or 128): plain fp16 cache, no quant / scale region
# =============================================================================
@triton.jit
def _fused_kv_compress_norm_rope_insert_fp16(
    # ── state cache (compressor internal state) ──
    state_cache_ptr,
    state_cache_stride0,
    state_cache_stride1,
    ...
    OVERLAP: tl.constexpr,
    ROPE_HEAD_DIM: tl.constexpr,
    KV_BLOCK_STRIDE: tl.constexpr,  # in fp16 elements
):
    """ROCm fp16 variant: compress → RMSNorm → RoPE → fp16 store.

    One program per token; early-exits for non-boundary positions.

    Bit-identical compression math (softmax weighted sum, fp32 RMSNorm,
    GPT-J interleaved RoPE) to the FP8 kernels, but writes the full
    normed+rotated vector as fp16 at row ``kv_pos_in_block * HEAD_SIZE``
    of a plain [num_blocks, block_size, HEAD_SIZE] fp16 cache — no FP8
    quantization and no scale region.
    """
    token_idx = tl.program_id(0)
    ...
    # ── KV cache pointers ────────────────────────────────────────────
    # kv_slot_mapping is in compressed-UNIT space (see
    # get_compressed_slot_mapping in the attention metadata builder):
    # slot = block_table[req][unit // units_per_block] * units_per_block
    #        + unit % units_per_block, with unit = position // COMPRESS_RATIO.
    kv_slot_idx = tl.load(kv_slot_mapping_ptr + token_idx)
    if kv_slot_idx < 0:
        return
    kv_block_idx = kv_slot_idx // kv_cache_block_size
    ...
    # ── Plain fp16 store: token stride == HEAD_SIZE, no scale region ──
    tl.store(out_ptr + block, result.to(tl.float16), mask=mask)
