...
# ---------------------------------------------------------------------------
# GLM53-PORT: local shims replacing upstream-only helpers/op classes.
# ---------------------------------------------------------------------------
def _get_moe_router_dtype(config) -> torch.dtype | None:
    """Fork-local copy of deepseek_v2._get_moe_router_dtype (absent here).

    GLM-5.3-Flash configs do not carry ``moe_router_dtype``; routing defaults
    to the FusedMoE/GateLinear defaults.
    """
...
# mHC (hyper-connection) helpers. Upstream implements these as CustomOp
# classes (MHCPreOp/MHCPostOp/MHCFusedPostPreOp + hc_expand/hc_contract) with
# tilelang/aiter kernels; this fork already has (DSV4-validated) module-level
# custom ops torch.ops.vllm.mhc_pre / mhc_post with an fp32-safe torch
# fallback and an optional fused Triton path (VLLM_DSV4_MHC_TRITON). The
# shims below reproduce the upstream class ops' semantics on top of them:
#   * hc_fused_post_pre == hc_post followed by hc_pre (the fusion is a pure
#     performance view; numerics are identical).
#   * the fused-RMSNorm into the pre stage (norm_weight/norm_eps upstream
#     kwargs) is decomposed: the caller applies its RMSNorm module to the
#     returned layer_input separately.
...
class Glm5NextMLP(nn.Module):
    def __init__(
        ...
        # GLM53-PORT: sequence-parallel MoE is upstream-only; this fork always
        # uses standard TP with an allreduce at the end.
...
class Glm5NextMoE(nn.Module):
    def __init__(
        ...
        # GLM53-PORT: sequence-parallel MoE requires upstream's
        # vllm.models.common.ops.sequence_parallel collectives, which do not
        # exist in this fork. SP-MoE is disabled; plain TP semantics.
        ...
        # GLM53-PORT: fork-local helper (see module top).
        ...
        # GLM53-PORT: upstream constructs the experts via FusedMoEFactory /
        # MoERunner (new API). This fork carries the older FusedMoE API where
        # the model computes router_logits itself and passes them into the
        # FusedMoE layer (same pattern as deepseek_v2 / hy_v3 in this fork).
    ...
    def forward(
        ...
        # GLM53-PORT: fork old-API FusedMoE - router logits computed here in
        # the model and fed into the experts layer (identical math to
        # upstream's externally-gated MoERunner; mirrors hy_v3.py).
...
class Glm5NextDecoderLayer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.mhc = config.mhc
        # GLM53-PORT: fp32 hc residual streams (same fix as fork DSV4 bug #1:
        # bf16-native hc streams overflow fp16). GEMM inputs are cast to the
        # model dtype after each pre-norm instead.
        self.model_dtype = vllm_config.model_config.dtype
        is_kda_layer = not is_mtp_layer and config.is_kda_layer(layer_idx)
        self.layer_kind = "kda" if is_kda_layer else "mla"
        self.is_sequence_parallel = False  # GLM53-PORT: SP-MoE unsupported
        ...
        if is_kda_layer:
            ...
            self.self_attn = Glm5NextMLAAttention(
                ...
                quant_config=(
                    quant_config
                    if isinstance(
                        quant_config, compressed_tensors.CompressedTensorsConfig
                    )
                    else None
                ),
                prefix=f"{prefix}.self_attn",
    ...
    def forward(
        ...
        # hc_post inputs (its ffn-pre outputs); when present, that hc_post is
        # applied first and then this layer's attn hc_pre runs (inter-layer
        # fusion upstream; sequential here - same numerics, GLM53-PORT).
        # Layer 0 has no incoming state -> standalone hc_pre.
        x = hidden_states
        if post is None:
            if self.layer_idx == 0:
        ...
        # Separate RMSNorm replaces upstream's norm fusion inside the pre op.
        x = self.input_layernorm(x)
        # GEMM kernels on gfx906 require model-dtype activations; the hc
        # residual stream itself stays fp32.
        ...
        # post-attn hc_post + pre-FFN hc_pre (fused upstream).
        residual, post, comb, x = self.hc_fused_post_pre(
            x,
            residual,
            post,
    ...
    def hc_pre(
        ...
        # GLM53-PORT: fork custom ops (torch fallback + optional fused Triton
        # via VLLM_DSV4_MHC_TRITON=1). Import registers mhc_pre AND mhc_post.
    ...
    def hc_fused_post_pre(
        ...
        # GLM53-PORT: decomposed into hc_post(residual update) + hc_pre on the
        # updated streams. Numerically identical to upstream's fused op.
        residual = self.hc_post(x, residual, post, comb)
        post, comb, layer_input = self.hc_pre(residual, hc_fn, hc_scale, hc_base)
...
class Glm5NextModel(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # GLM53-PORT: model dtype used for post-norm activation casting when
        # the hc residual stream runs in fp32.
        self.model_dtype = vllm_config.model_config.dtype
    ...
    def forward(
        ...
        **kwargs,
        ...
        if get_pp_group().is_first_rank:
            ...
            # GLM53-PORT: fp32 hc residual streams (fork DSV4 bug #1: bf16-
            # native hc streams overflow fp16). The embedding layer-0 hc_expand
            # runs in fp32; each decoder layer casts to the model dtype only
            # AFTER its pre-norms, before the GEMM-bound submodules.
            if self.config.mhc:
        ...
        # GLM53-PORT: streams are fp32; cast the final hidden states back to
        # the model dtype for the logits GEMM (mirrors fork deepseek_v4.py).
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for args in weights:
            ...
            name = name.replace(".attn_hc.", ".hc_attn_")
            name = name.replace(".ffn_hc.", ".hc_ffn_")
            ...
            # Native BF16/raw-FP8 weights keep the historical weight padding.
            # A shape-matched source destination uses CT activation padding,
            # including when that projection is explicitly ignored by CT.
            if (
                kv_a_pad_size > 0
                and ".kv_a_proj_with_mqa." in name
                and name.endswith(".weight")
                and (
                    name not in params_dict
                    or params_dict[name].size() != loaded_weight.size()
                )
            ):
                pad = torch.zeros(
                    kv_a_pad_size,
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                # Prefer a real source-named destination over a missing fusion.
                if name in params_dict and name_mapped not in params_dict:
                    continue
...
class Glm5NextForConditionalGeneration(
    ...
    def get_encoder_cudagraph_config(self):
        # GLM53-PORT: the fork's Glm4vForConditionalGeneration predates the
        # encoder-cudagraph plumbing (Protocol-only); skip instead of erroring.
        config_getter = getattr(super(), "get_encoder_cudagraph_config", None)
...
def _try_load_fp8_attn_proj(
    ...
    # Native FP8 uses weight_scale_inv; CT owns weight_scale and must be loaded
    # by the source projection's quantization-aware loader.
    source_prefix = name.rsplit(".", 1)[0]
    if (
        target_s in params_dict
        or f"{layer_prefix}.{target_base}.weight_scale" in params_dict
        or f"{source_prefix}.weight_scale" in params_dict
    ):
        return False
