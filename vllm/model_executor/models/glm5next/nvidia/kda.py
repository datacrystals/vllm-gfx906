...
# GLM53-PORT: upstream gates non-graph-safe host branches behind
# eager_break_during_capture (breakable cuda-graph split points). This fork
# runs the sparse/mamba hybrid path eagerly (DSV4 precedent), so a no-op
# decorator keeps the call sites intact.
...
if current_platform.is_rocm():
    from vllm.model_executor.models.glm5next.amd.ops.third_party.kda import (
        chunk_kda_with_fused_gate,
        fused_recurrent_kda,
    )
else:
    from vllm.model_executor.models.glm5next.nvidia.ops.third_party.kda import (
        chunk_kda_with_fused_gate,
        fused_recurrent_kda,
    )
...
class Glm5NextLinearAttention(GatedDeltaNetAttention):
    ...
    def get_state_shape(
        ...
        # GLM53-PORT: fork's kda_state_shape covers KimiLinear's 3 separate
        # conv states; GLM-5.3 stores ONE merged (q|k|v) short-conv state, so
        # the shape is computed here directly (upstream formula).
        # conv_state width must include num_spec so the spec-decode conv update
        # can slide the window across draft-verify tokens without reading past
        # the allocated width.
        conv_state_shape = MambaStateShapeCalculator._orient_conv_shape(
            3 * (self.head_dim * self.num_heads) // self.tp_size,
            self.conv_size - 1 + self.num_spec,
        )
    ...
    def __init__(
        ...
        vllm_quant_config = self.quant_config
        self.unfused_projections = isinstance(
            self.quant_config, compressed_tensors.CompressedTensorsConfig
        )
        # GLM53-PORT: (cyankiwi CT policy) always keep the CT quant config so
        # its ignore list can dynamically fall back to dense BF16 for the KDA
        # projections; native FP8 checkpoints instead zero it (upstream ORIG
        # behavior) since those BF16 projections carry no scales.
        ...
        if self.unfused_projections:
            ...
            # Preserve the native fused fast path and its replicated shard loader.
        ...
        if not self.unfused_projections:
            # Native FP8 checkpoints omit scales for these BF16 projections.
        ...
        self.o_proj = RowParallelLinear(
            ...
            self.hidden_size,
            bias=False,
            # GLM53-PORT: KDA o_proj is on the CT ignore list (BF16); passing
            # the original CT config lets the ignore-name fallback keep it
            # dense instead of assuming native-FP8 unquantized semantics.
    ...
    def forward(
        ...
        if self.unfused_projections:
            ...
            g_a = self.g_a_proj(hidden_states)[0]
        else:
