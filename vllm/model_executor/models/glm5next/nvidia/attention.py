...
# GLM53-PORT: fwht128_quant_fp8 is only used on the fp8 Hadamard path
# (CUDA). On ROCm/gfx906 the indexer runs fp16 and never rotates/quantizes.
try:
    from vllm.model_executor.models.glm5next.nvidia.ops.kpool_compress import (
        fwht128_quant_fp8,
    )
except Exception:  # pragma: no cover
    fwht128_quant_fp8 = None
...
class Glm5NextIndexerCache(DeepseekV32IndexerCache):
    ...
    GLM53-PORT: upstream main encodes kpool via ``tokens_per_state`` on
    MLAAttentionSpec; this fork's DSV4 machinery instead uses
    ``compress_ratio`` on the spec, which makes the (DSV4-proven, bugfixed)
    indexer metadata builder emit pool-granular ``slot_mapping`` /
    ``seq_lens`` / ``cu_seq_lens`` / ``page_table`` for free and shrinks the
    cache allocation to one state per ``index_kpool`` tokens.

    On ROCm/gfx906 (VLLM_GLM53_INDEXER_FP16, default ON) the cache holds raw
    fp16 K entries (head_dim floats, no fp8 scale), replacing upstream's
    Hadamard-rotated fp8(+scale) cache -- gfx906 has no fp8 compute.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            head_dim=head_dim,
            dtype=torch.float16 if use_fp16 else dtype,
            prefix=prefix,
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig):
        ...
        # Pool-granular storage pages must tile in 32s so the paged fp16
        # MQA-logits kernels (fork family: block 64) can address them.
        assert (
            ...
            f"be a multiple of index_kpool * 32 ({self._index_kpool * 32}), "
...
class Glm5NextTailCache(DeepseekV32IndexerCache):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig):
        ...
        return KpoolTailSpec(
            ...
            head_size_v=0,
            # GLM53-PORT: use the cache dtype (fp16 on gfx906 fp16 path).
            dtype=self.dtype,
...
class Indexer(nn.Module):
    def __init__(
        ...
        # GLM53-PORT: on ROCm/gfx906 the indexer runs fp16 end to end; the
        # K cache holds raw fp16 entries (no scale bytes). CUDA keeps the
        # upstream fp8(+scale) uint8 layout.
        self.use_fp16 = bool(
            current_platform.is_rocm() and envs.VLLM_GLM53_INDEXER_FP16
        )
        if self.use_fp16:
        ...
        # GLM53-PORT: on the fp16 path the tail cache matches the model K
        # dtype (fp16) instead of upstream's hardcoded bf16.
    ...
    def forward(
        ...
        if self.use_fp16:
            # GLM53-PORT fp16 path: no Hadamard rotation and no fp8 quant.
            # The rotation is an orthogonal transform (q.H @ H.k == q.k) that
            # only exists to spread the fp8 quant error; the fp16 cache keeps
            # the exact logits. The per-token q scale is 1, so weights only
            # fold the softmax + head-count scales.
            q_indexer = q.to(torch.float16).view(-1, self.n_head, self.head_dim)
            ...
            q_indexer = q_fp8.view(-1, self.n_head, self.head_dim)
        ...
        # GLM53-PORT: cast the (bf16 checkpoint) gate to the activation dtype;
        # upstream assumes a bf16 model.
        gate_score = F.linear(
            hidden_states,
        ...
        # GLM53-PORT: the fork fp16 triton kernels only require heads % 8 == 0
        # (v4 path; v2 fallback handles the rest), so GLM-5.3's 32 indexer
        # heads need no padding.
        if not self.use_fp16 and self.n_head < 32:
            pad = 32 - self.n_head
            q_indexer = _pad_indexer_heads(q_indexer, pad)
            weights = _pad_indexer_heads(weights, pad)

        if self.use_fp16:
            # tail cache stores K as fp16 next to the fp16 indexer cache.
            pass
...
class Glm5NextMLAAttention(nn.Module):
    def __init__(
        ...
        ct_projections = isinstance(
            quant_config, compressed_tensors.CompressedTensorsConfig
        )
        self.fused_qkv_a_proj: nn.Module | None = None
        self.q_a_proj: nn.Module | None = None
        self.kv_a_proj_with_mqa: nn.Module | None = None
        if self.q_lora_rank is not None and ct_projections:
            ...
            kv_a_output_size = self.kv_lora_rank
            if not skip_rope:
                kv_a_output_size += self.qk_rope_head_dim
        ...
        self.mla_attn = MultiHeadLatentAttentionWrapper(
            ...
            prefix,
            skip_topk=False,
            # GLM53-PORT: fork's MLA wrapper has no fused q/kv RMSNorm kernel
            # flag; it always applies q_a/kv_a RMSNorms separately, which is
            # numerically identical to upstream's fused launch.
        )
