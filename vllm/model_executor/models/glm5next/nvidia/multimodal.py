...
# GLM53-PORT: fork has no vllm.models.common.ops; the DSV4 fused q/k RMSNorm
# triton op is API-compatible for this call shape (flat [tokens, dim]).
...
class Glm5NextVisionTransformer(nn.Module):
    # GLM53-PORT: this fork's WeightsMapper/AutoWeightsLoader predate
    # orig_to_new_stacked. The GLM-5.3 checkpoint already stores fused
    # attn.qkv; only gate_up needs merging, handled by stacked_params_mapping
    # in load_weights below instead.
    ...
    def load_weights(self, weights) -> set[str]:
        ...
        # GLM53-PORT: fork-style stacked-params merge (see class comment).
        ...
        for name, w in weights:
            for pname, wname, shard_id in stacked_params_mapping:
                if wname in name:
                    name = name.replace(wname, pname)
                    params_dict[name].weight_loader(params_dict[name], w, shard_id)
                    loaded.add(name)
                    break
            else:
...
class Glm5NextProcessingInfo(Glm4vProcessingInfo):
    ...
    def _processor_pixel_budget(
        self, proc, modality: str | None = None
    ) -> tuple[int, int]:
        ...
        if min_tokens is None:
            min_tokens = proc.min_image_tokens
        if max_tokens is None:
            max_tokens = proc.max_image_tokens
    ...
    def _get_image_max_pixels(self) -> int:
        ...
        return self._processor_pixel_budget(
            self.get_hf_processor().image_processor, "image"
        )[1]

    def get_num_frames_with_most_features(
        self, seq_len: int, mm_counts: Mapping[str, int]
    ) -> int:
        if not mm_counts.get("video", 0):
            return 1
    ...
    def _get_video_max_pixels(self) -> int:
        ...
        if (override := mm_kwargs.get("max_pixels")) is not None:
            return int(override)
        return self._processor_pixel_budget(
            self.get_hf_processor().video_processor, "video"
        )[1]
