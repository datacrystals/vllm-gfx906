...
# ---------------------------------------------------------------------------
# GLM53-PORT: fp16-cache variants for gfx906 (no fp8 compute). The pooled K
# vector is stored directly in fp16; the Hadamard rotation + fp8 absmax scale
# steps are skipped (they only exist to bound fp8 SNR). Layout: cache is
# [num_blocks, page_size, head_dim] fp16, NO preshuffle, NO scale region.
# The pooling math (softmax(gate+ape)-weighted sum in fp32) is identical to
# the fp8 path.
# ---------------------------------------------------------------------------


@triton.jit
def _kpool_softmax_pool_write_fp16_kernel(
    buf_fp16_ptr,
    slot_k_ptr,
    slot_score_ptr,
    ape_ptr,
    ...
    """One program per pool. softmax(slot_score+ape)-weighted sum of slot_k,
    stored as fp16 at the pool slot ``loc``."""
    row = tl.program_id(0)
    do_write = True
    if HAS_WRITE_MASK:
        do_write = tl.load(write_mask_ptr + row)
    ...
    tl.store(buf_fp16_ptr + out_offset, x.to(tl.float16), mask=mask)


def kpool_compress_and_write_cache_fp16(
    kv_cache: torch.Tensor,
    slot_k: torch.Tensor,
    slot_score: torch.Tensor,
    ape: torch.Tensor,
    ...
    """fp16 variant of :func:`kpool_compress_and_write_cache` (GLM53-PORT).

    Args:
        kv_cache: indexer K cache ``[num_blocks, page_size, head_dim]`` fp16.
        slot_k: ``[n_pools, pool_size, head_dim]`` — raw per-token K.
        slot_score: ``[n_pools, pool_size, head_dim]`` — per-token gate score.
        ape: ``[pool_size, head_dim]`` fp32 — per-slot position bias.
        loc: ``[n_pools]`` int64 — flat physical slot per pool.
    """
    ...
    _kpool_softmax_pool_write_fp16_kernel[(slot_k.shape[0],)](
        kv_cache,
        slot_k,
        slot_score,
        ape,
...
@triton.jit
def _kpool_decode_update_batched_fp16_kernel(
    buf_fp16_ptr,
    tail_kv_ptr,
    tail_slot_mapping_ptr,  # [B, NEXT_N] int32
    key_ptr,  # [B, NEXT_N, HEAD_DIM] fp16
    key_stride_b,
    key_stride_t,
    slot_score_ptr,  # [B, NEXT_N, HEAD_DIM] fp16
    ss_stride_b,
    ss_stride_t,
    ape_ptr,
    ape_stride_0,
    ...
    """fp16 variant of ``_kpool_decode_update_batched_kernel`` (GLM53-PORT):
    identical pool/tail semantics, but completed pools are stored as fp16 with
    no Hadamard rotation and no per-vector fp8 scale.
    """
    req = tl.program_id(0)
    offs = tl.arange(0, BLOCK_D)
    dim_mask = offs < HEAD_DIM
    ...
    for t in tl.range(0, NEXT_N):
        ...
        if pos_valid & (slot == POOL_SIZE - 1):
            ...
            tl.store(
                buf_fp16_ptr + out_offset,
                x.to(tl.float16),
                mask=dim_mask & pos_valid,
            )

        # Stash the current token AFTER any completion read so the completion
        # uses prior stashes (and the current token's own key/score via
        # is_current), then leaves this token for future pools.
        update_mask = dim_mask & stash_valid
        tl.store(
            tail_kv_ptr + block_base + phys_slot * HEAD_DIM + offs,
            key.to(tail_kv_ptr.dtype.element_ty),
            mask=update_mask,
        )
        tl.store(
            tail_kv_ptr + block_base + KPOOL_HEAD + phys_slot * HEAD_DIM + offs,
            score_current.to(tail_kv_ptr.dtype.element_ty),
            mask=update_mask,
        )


def kpool_decode_update_and_maybe_write_cache_batched_fp16(
    kv_cache: torch.Tensor,
    tail_kv_cache: torch.Tensor,
    tail_slot_mapping: torch.Tensor,
    key: torch.Tensor,
    ...
    """fp16 variant of ``kpool_decode_update_and_maybe_write_cache_batched``
    (GLM53-PORT). ``kv_cache`` is fp16; ``tail_kv_cache`` matches ``key``
    dtype (fp16 on the gfx906 fp16 path instead of upstream's bf16).

    Args:
        kv_cache: indexer K cache ``[num_blocks, page_size, head_dim]`` fp16.
        tail_kv_cache: paged tail cache ``[num_blocks, 2, pool_size, head_dim]``
            (K at half 0, gate score at half 1).
        tail_slot_mapping: ``[num_requests, next_n]`` int32.
        key: ``[num_requests, next_n, head_dim]``.
        slot_score: ``[num_requests, next_n, head_dim]``.
        ape: ``[pool_size, head_dim]`` fp32.
        slot_mapping: ``[num_requests, next_n]`` int32.
        positions: ``[num_requests, next_n]`` int32.
    """
    ...
    _kpool_decode_update_batched_fp16_kernel[(num_requests,)](
        kv_cache,
        tail_kv_cache,
        tail_slot_mapping,
        key,
...
def kpool_seed_tail_cache_generic(
    tail_kv_cache: torch.Tensor,
    key: torch.Tensor,
    gate_score: torch.Tensor,
    tslot: torch.Tensor,
    ...
    """GLM53-PORT: dtype-generic variant of :func:`kpool_seed_tail_cache`
    (upstream asserts bf16; the gfx906 fp16 path keeps K in fp16)."""
    assert key.dtype == tail_kv_cache.dtype
    assert gate_score.dtype == tail_kv_cache.dtype
    n = tslot.shape[0]
    if n == 0:
        return
    _kpool_tail_seed_kernel[(n,)](
