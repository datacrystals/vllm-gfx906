...
# GLM53-PORT: fork's fla op module predates exp2; define it locally
# (upstream refactor added it alongside exp/log).
from vllm.model_executor.layers.fla.ops.op import exp, log  # noqa: F401
from vllm.triton_utils import tl as _tl_glm53

# exp2 may be missing on the placeholder tl (no-GPU import); resolved
# properly when real triton compiles the kernel.
if hasattr(_tl_glm53, "exp2"):
    exp2 = _tl_glm53.exp2
else:

    def exp2(x):
        return _tl_glm53.exp(x * 1.4426950408889634)
from vllm.model_executor.layers.fla.ops.solve_tril import solve_tril
from vllm.model_executor.layers.fla.ops.utils import FLA_CHUNK_SIZE, is_amd
